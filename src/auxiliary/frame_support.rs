...
pub mod storage {
    ...
    pub trait StorageMap<Twox64Concat, K, V> {
        ...
        /// Mutate the item, only if an `Ok` value is returned. Deletes the item if mutated to a
        /// `None`.
    ...
    pub trait StorageDoubleMap<K1, K2, V> {
        ...
        /// completly ignored by this function.
    ...
    pub trait IterableStorageDoubleMap<K1, K2, V>: StorageDoubleMap<K1, K2, V> {
        ...
        /// Enumerate all elements in the map with first key `k1` in no particular order. If you add
        /// or remove values whose first key is `k1` to the map while doing this, you'll get
        /// undefined results.
        ...
        /// Translate the values of all elements by a function `f`, in the map in no particular
        /// order. By returning `None` from `f` for an element, you'll remove it from the
        /// map.
        ///
